...
const STATUS_LABEL: Record<string, { text: string; cls: string }> = {
  ...
  PAID: { text: "Confirmed", cls: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300" },
...
function Row({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="flex justify-between gap-3 text-sm py-1">
      ...
      <span className="font-medium text-slate-900 dark:text-slate-100 text-right break-all">{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-lg glass-panel p-6 sm:p-8">
        <h1 className="font-display text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 mb-1">
          Registration Status
        </h1>
        ...
        {data?.found && (
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono font-semibold text-slate-900 dark:text-slate-100">{query}</span>
            </div>
            {status && (
            ...
            {data.status === "PENDING" && (
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 p-4 space-y-3">
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">How to pay</p>
                ...
                {qrDataUrl && (
                  <div className="flex flex-col items-center pt-2">
                    ...
                    <p className="text-[11px] text-slate-500 mt-1">Scan to reopen this status page</p>
                  </div>
                )}
              </div>
            )}
            ...
            {data.status === "PAID" && (
              <div className="flex flex-col items-center gap-2">
                <p className="text-sm text-slate-600 dark:text-slate-400">Payment received — your registration is confirmed.</p>
                {data.invoiceReady && (
