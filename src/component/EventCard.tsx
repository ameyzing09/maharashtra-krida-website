...
const EventCard: React.FC<EventProps> = ({
  ...
  return (
    <Link to={`/upcoming-events/${id}`} className="block w-full h-full" aria-label={`View details for ${name}`}>
      <motion.article
      ...
      >
        ...
        <div className="flex flex-col h-full p-4 sm:p-5 relative">
          ...
          <div className="relative z-10 flex flex-col h-full">
            ...
            {/* View Details indicator - pushed to bottom */}
            <div className="mt-auto">
              <span className="text-sm text-lime-600 dark:text-lime-400 group-hover:underline transition-all duration-200">
                View Details &rarr;
              </span>
            </div>
          </div>
